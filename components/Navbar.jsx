    ...
    >
      ...
      <button onClick={() => setMenuOpen(!menuOpen)} className=" duration-200 relative h-7 z-30 border-red-300">
        {menuOpen ? (
          ...
          <>
            <div className={`h-1 w-7 absolute top-1 left-0 ${menuOpen ? 'bg-gray-200' : ' bg-gray-700'}`} />
            <div className={`h-1 w-7 relative top-0 ${menuOpen ? 'bg-gray-200' : ' bg-gray-700'}`} />
            <div className={`h-1 w-7 absolute top-5 left-0 bg-gray-700 ${menuOpen ? 'bg-gray-200' : ' bg-gray-700'}`} />
          </>
        )}
      </button>
      {menuOpen && (
        <div className="fixed top-0 left-0 bg-gray-900 w-full h-full z-20  p-4">
          <nav className="flex flex-col gap-3 mt-20 font-bold text-4xl">
            <Link href="/"> Home </Link>
